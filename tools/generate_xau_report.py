...
def main() -> int:
    res = run_analysis("GC1!", "15m", bar_count=80, with_ai=True)
...
<h1>XAU · 黄金 15分钟威科夫分析</h1>
<p class="meta">生成时间: {now} | 数据源: MT5 (XAUUSD CFD) | 周期: 15m | K线数: 80 | AI: {res.usage.get('total_tokens', 'N/A')} tokens</p>
...
<div class="disclaimer">⚠️ 订单流经 MT5 CFD (XAUUSD) 获取，tick 仅含 bid/ask，方向按 Tick Rule 近似分类，成交量为 tick 计数近似值。威科夫判断为概率化场景，不构成投资建议。</div>
    ...
    path = OUTPUT_DIR / f"xau_15m_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
